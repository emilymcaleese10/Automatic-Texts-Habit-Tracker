...
async def handle_message(update: Update, context: CallbackContext) -> None:
    ...
    user_name = update.message.from_user.username or f"User_{user_id}"
    user_message = update.message.text.strip()

    user_key = f"{user_name}_{user_id}"

    if user_key not in user_conversations:
        user_conversations[user_key] = []  # Create a new conversation list for the user if it doesn't exist   
    ...
    user_conversations[user_key].append({"User": user_message, "Timestamp": str(update.message.date)})  # user message
    user_conversations[user_key].append({"Bot": bot_message}) # bot message
